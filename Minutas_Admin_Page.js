...
const MinutasAdminPage = () => {
    ...
    // Efeito principal para carregar os dados da página
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            setError('');
            try {
                // 1. Busca setores e tipos de minuta em paralelo
                const [setoresSnapshot, tiposSnapshot] = await Promise.all([
                    db.collection('setores').orderBy('nome').get(),
                    db.collection('minutaTipos').orderBy('nome').get()
                ]);
                ...
                // 2. Define o setor inicial
                let initialSetorId = '';
                if (userData.role === 'setor_admin') {
                    initialSetorId = userData.setorId;
                } else if (setoresList.length > 0) {
                    initialSetorId = setoresList[0].id;
                }
                setSelectedSetorId(initialSetorId);

                // 3. Se houver um setor inicial e tipos de minuta, busca as minutas
                if (initialSetorId && tiposList.length > 0) {
                    const minutasData = {};
                    await Promise.all(tiposList.map(async (tipo) => {
                        const docRef = db.collection('minutas').doc(`${initialSetorId}_${tipo.id}`);
                ...
                setError('Falha ao carregar dados da página de minutas.');
                console.error(err);
            } finally {
                setLoading(false);
            }
    ...
    // Efeito para buscar as minutas QUANDO o setor selecionado muda (para Admins)
    useEffect(() => {
        // Este efeito é para quando o admin troca de setor no dropdown.
        // A busca inicial já é feita no primeiro useEffect.
        // A verificação `useRef` previne a execução na primeira renderização.
        if (isInitialMount.current) {
            isInitialMount.current = false;
        } else {
            const fetchMinutasForSetor = async () => {
                if (!selectedSetorId || !db || tiposMinuta.length === 0) return;
                setLoading(true);
                try {
                    const minutasData = {};
            ...
            fetchMinutasForSetor();
        }
    }, [selectedSetorId, db, tiposMinuta]); // Roda quando o setor muda
    ...
    const handleAddTipoMinuta = async (e) => {
        ...
        try {
            ...
            window.location.reload(); // Força o recarregamento para ver a alteração
        } catch (err) {
        ...
        <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl p-6 sm:p-8 rounded-2xl shadow-lg border border-slate-200/50 dark:border-slate-700/50 space-y-6">
            ...
            {loading ? (
                ...
                <div className="space-y-8">
                    {/* CORREÇÃO: Troca a constante 'TIPOS_MINUTA' pela variável de estado 'tiposMinuta' */}
